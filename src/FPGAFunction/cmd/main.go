...
import (
	...
	"FPGAFunction/internal/controller"
...
func main() {
	...
	/* metric server port */
	ms_port := os.Getenv("K8S_FPGA_MS_PORT")
	if ms_port == "" {
		ms_port = "8080"
	}

	/* health check server port */
	hc_port := os.Getenv("K8S_FPGA_HC_PORT")
	if hc_port == "" {
		hc_port = "8081"
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "d13595bf.example.com",
	...
	if err = (&controller.FPGAFunctionReconciler{
		...
		Recorder: mgr.GetEventRecorderFor("fpgafunction-ctl"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "FPGAFunction")
		os.Exit(1)
	}
