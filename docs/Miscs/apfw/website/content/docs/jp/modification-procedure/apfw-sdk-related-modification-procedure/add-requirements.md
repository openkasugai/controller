...
## 対応内容
* `requirements.in`と`requirements.txt`を新規作成し、Kubeflow Pipelines SDK実行時に使用するPythonパッケージである`kfp-pipeline-spec`を、公開されているバージョンから独自に改修したものを参照するように変更し、pipの依存関係から除外する。

## 前提条件
* 当該手順を実施する前に以下手順を実施済みであること。
    * [OpenKasugaiライブラリ追加](../add-dci-library)

## 手順
1. `requirements.in`を新規作成する。
```
