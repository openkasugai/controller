...
# buildah bud run
buildah bud -t ${IMAGE_NAME} -f ${CPU_DECODE_DIR}/docker/Dockerfile --build-arg CPU_DECODE_DIR=${CPU_DECODE_DIR} --build-arg FPGA_SOFTWARE_DIR=${FPGA_SOFTWARE_DIR} --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy 
