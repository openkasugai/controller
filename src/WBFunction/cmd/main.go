...
import (
	...
	"WBFunction/internal/controller"
...
func main() {
	...
	/* metric server port */
	ms_port := os.Getenv("K8S_WBF_MS_PORT")
	if ms_port != "" {
		metricsAddr = ":" + ms_port
	}
	...
	hc_port := os.Getenv("K8S_WBF_HC_PORT")
	if hc_port != "" {
		probeAddr = ":" + hc_port
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "e6c1a2e4.example.com",
	...
	if err = (&controller.WBFunctionReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("wbfunction-ctrl"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "WBFunction")
		os.Exit(1)
	}

	//+kubebuilder:scaffold:builder
	controller.LoadConfigMap(&controller.WBFunctionReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("wbfunction-ctrl"),
	})
